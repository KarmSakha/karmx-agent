...
fn main() {
    ...
    let karmx = std::env::var("KARMX_BIN")
        .or_else(|_| std::env::var("GOOSE_BIN"))
        .unwrap_or_else(|_| "target/debug/karmx".to_string());
    let path_root = tempfile::Builder::new()
        .prefix("karmx-mcp-conformance-")
        .tempdir()
        .unwrap_or_else(|err| {
            eprintln!("failed to create temporary KARMX_PATH_ROOT: {err}");
            std::process::exit(1);
        });
    let mut child = Command::new(&karmx)
        .args(["mcp-probe", target, "--script", "-"])
        .env("KARMX_OAUTH_AUTOMATIC_CALLBACK", "1")
        .env("KARMX_DISABLE_KEYRING", "1")
        .env("KARMX_PATH_ROOT", path_root.path())
        .stdin(Stdio::piped())
        .spawn()
        .unwrap_or_else(|err| {
            eprintln!("failed to spawn {karmx}: {err}");
            std::process::exit(1);
        });
    ...
    child
        ...
        .expect("write probe script to karmx stdin");

    let status = child.wait().expect("wait for karmx");
