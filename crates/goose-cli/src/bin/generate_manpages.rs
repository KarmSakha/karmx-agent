//! Generate manpages for the karmx CLI.
//!
...
//! offline documentation accessible via the `man` command (e.g., `man karmx`).
//!
//! When karmx is packaged for Linux distributions (deb, rpm, etc.), the generated
...
//!
//! Output: target/man/karmx.1, target/man/karmx-session.1, etc.
...
fn generate_see_also(
    ...
    // Always reference the main karmx command if we're not it
    if current_name != "karmx" {
        references.push("karmx".to_string());
    }

    // Reference parent command if exists and not already added
    if let Some(parent) = parent_name {
        if parent != "karmx" && !references.contains(&parent.to_string()) {
    ...
    if current_name == "karmx" {
        ...
        for subcmd in cmd.get_subcommands() {
            ...
            if subcmd_name != "help" && !subcmd.is_hide_set() {
                let full_name = format!("karmx-{}", subcmd_name);
