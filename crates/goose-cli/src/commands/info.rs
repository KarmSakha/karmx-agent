...
pub async fn handle_info(verbose: bool, check: bool) -> Result<()> {
    ...
    println!("{}", style("karmX Version:").cyan().bold());
    ...
    if verbose {
        println!("\n{}", style("karmX Configuration:").cyan().bold());
        let values = config.all_values()?;
        if values.is_empty() {
            println!("  No configuration values set");
            println!(
                "  Run '{}' to configure karmx",
    ...
    if check {
        ...
        match &result {
            ...
            Err(ProviderCheckError::NotConfigured { label, error }) => {
                ...
                print_aligned(
                    "Hint:",
                    &format!("Run '{}'", style("karmx configure").cyan()),
                    label_padding,
                );
            }
        ...
        // checks, health probes) can rely on `karmx info --check` as a
        // pre-flight verifier.
        if result.is_err() {
