//! Deterministic, Rust-driven orchestration for `karmx review`.
//!
...
//!
//! - One subprocess per check (`karmx run -q -t <prompt>`)
...
pub async fn run_checks_in_parallel(
    ...
    while let Some(joined) = set.join_next().await {
        let (idx, check, result, quiet) = match joined {
            Ok(v) => v,
            Err(e) => {
                eprintln!("karmx review: check task panicked: {e}");
                continue;
            }
        };
        ...
        match result {
            Ok(findings) => {
                if !quiet {
                    eprintln!(
                        "karmx review: check '{}' completed: {} finding(s)",
                        check.name,
                        findings.len()
                    );
                }
            ...
            Err(e) => {
                ...
                eprintln!("karmx review: check '{}' failed: {e}", check.name);
                results[idx] = Vec::new();
            }
        }
    }
...
///    asks for `karmx-claude-4-sonnet` would 404 against Google's API),
...
///
/// Uses `karmx review --turn-limit` when set, otherwise
...
/// Spawn a single `karmx run` subprocess for one check and parse its
...
/// Generic `karmx run` subprocess that hands a prompt to the model
...
async fn run_subprocess_for_findings(
    ...
    let goose_bin = std::env::current_exe().context("locate current karmx binary")?;
    ...
    if let Some(mut stdin) = child.stdin.take() {
        ...
        // Closing stdin signals EOF to `karmx run -i -`.
        drop(stdin);
    }
...
pub async fn run_main_pass_in_parallel(
    ...
    while let Some(joined) = set.join_next().await {
        let (idx, path, result, quiet) = match joined {
            Ok(v) => v,
            Err(e) => {
                eprintln!("karmx review: main-pass task panicked: {e}");
                continue;
            }
        };
        match result {
            Ok(raw) => {
                ...
                if !quiet {
                    eprintln!(
                        "karmx review: main pass on '{}' completed: {} finding(s)",
                        path,
                        findings.len()
                    );
                }
            ...
            Err(e) => {
                ...
                eprintln!("karmx review: main pass on '{}' failed: {e}", path);
...
/// cap enforced by karmx. Without this, models routinely burn turns on
...
fn build_subprocess_turn_budget_section(max_turns: usize) -> String {
    format!(
        "## Turn budget\n\n\
         ...
         karmx enforces this via `--max-turns`; when you exhaust it, the session stops and \
...
mod tests {
    ...
    fn resolve_check_model_cli_provider_wins_over_per_check_model() {
        let mut c = ck("perf");
        c.model = Some("karmx-claude-4-sonnet".into()); // wrong provider
        let opts = ReviewOptions {
    ...
    fn resolve_check_model_cli_provider_alone_drops_per_check_model() {
        ...
        let mut c = ck("perf");
        c.model = Some("karmx-claude-4-sonnet".into());
        let opts = ReviewOptions {
            provider: Some("google".into()),
            default_model: None,
            ..ReviewOptions::default()
