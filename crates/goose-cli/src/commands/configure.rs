...
pub async fn handle_configure() -> anyhow::Result<()> {
    if !std::io::stdin().is_terminal() {
        anyhow::bail!(
            "karmx configure requires an interactive terminal.\n\
             If you installed via 'curl ... | bash', run 'karmx configure' separately after installation."
        );
    }
...
pub fn configure_telemetry_consent_dialog() -> anyhow::Result<bool> {
    ...
    println!();
    println!("{}", style("Help improve karmX").bold());
    println!();
    println!(
        "{}",
        style("Would you like to help improve karmX by sharing anonymous usage data?").dim()
    );
    println!(
        "{}",
        style("This helps us understand how karmX is used and identify areas for improvement.")
            .dim()
    );
    println!();
    ...
    println!("{}", style("  • karmx version and install method").dim());
    ...
    println!(
        "{}",
        style("or any personal data. You can change this anytime with 'karmx configure'.").dim()
    );
    println!();

    let enabled = cliclack::confirm("Share anonymous usage data to help improve karmX?")
        .initial_value(true)
        .interact()?;
    ...
    if enabled {
        let _ = cliclack::log::success("Thank you for helping improve karmX!");
    } else {
...
async fn handle_first_time_setup(config: &Config) -> anyhow::Result<()> {
    println!();
    println!("{}", style("Welcome to karmX! Let's get you set up.").dim());
    println!(
        "{}",
    ...
    println!();
    cliclack::intro(style(" karmx-configure ").on_cyan().black())?;
...
async fn handle_existing_config() -> anyhow::Result<()> {
    ...
    let action = cliclack::select("What would you like to configure?")
        ...
        .item(
            "sidekick",
            "Sidekick (fusion) model",
            "Pick the provider and model your sidekick runs on",
        )
        .item(
            "settings",
            "karmx settings",
            "Set the karmx mode, Tool Output, Tool Permissions, Experiment, karmx recipe github repo and more",
        )
        .interact()?;
    ...
    match action {
        ...
        "sidekick" => configure_sidekick_dialog().await,
    ...
    match config.set_secret(key_name, &value) {
        ...
        Err(e) => {
            cliclack::outro(style(format!(
                "Failed to store {} securely: {}. Please ensure your system's secure storage is accessible. Alternatively you can run with KARMX_DISABLE_KEYRING=true or set the key in your environment variables",
                key_name, e
            )).on_red().white())?;
            Ok(false)
        }
...
pub async fn configure_provider_dialog() -> anyhow::Result<bool> {
    ...
    let model: String = match models_res {
        ...
        Ok(_) => {
            let default_model = goose::config::env_var("GOOSE_MODEL")
    ...
    let toolshim_enabled = goose::config::env_var("GOOSE_TOOLSHIM")
        .map(|val| val == "1" || val.to_lowercase() == "true")
        .unwrap_or(false);
    let toolshim_model = goose::config::env_var("GOOSE_TOOLSHIM_OLLAMA_MODEL").ok();
...
/// Pick which provider + model the fusion sidekick runs on.
/// Persisted as KARMX_SIDEKICK_PROVIDER / KARMX_SIDEKICK_MODEL in config.yaml.
pub async fn configure_sidekick_dialog() -> anyhow::Result<()> {
    let config = Config::global();

    if goose::config::env_var("KARMX_SIDEKICK_PROVIDER").is_ok()
        || goose::config::env_var("KARMX_SIDEKICK_MODEL").is_ok()
    {
        let _ = cliclack::log::warning(
            "KARMX_SIDEKICK_* environment variables are set and take precedence over this choice.",
        );
    }

    // Candidates are providers already configured (credentials in place) so the
    // sidekick can actually run; the lead provider is always a candidate.
    let mut names: Vec<String> = config
        .get_param::<serde_json::Value>("providers")
        .ok()
            ...
            map.iter()
                .filter(|(_, e)| {
                    e.get("configured")
                        ...
                        .unwrap_or(false)
                        && e.get("enabled").and_then(|c| c.as_bool()).unwrap_or(true)
                })
                .map(|(k, _)| k.clone())
                .collect()
        })
        .unwrap_or_default();
    if let Ok(lead) = config.get_goose_provider() {
    ...
    const SAME_AS_LEAD: &str = "__same_as_lead__";
    let provider = {
        let mut select = cliclack::select("Sidekick provider").filter_mode();
        select = select.item(
            SAME_AS_LEAD.to_string(),
            "same as lead",
            "sidekick shares the lead's provider and model",
        );
        for name in &names {
    ...
    if provider == SAME_AS_LEAD {
        ...
        let _ = cliclack::log::success("Sidekick now follows the lead provider and model");
        return Ok(());
    }

    let _ = cliclack::log::info("Fetching models for the sidekick provider...");
    let models = match create(&provider, Vec::new()).await {
        Ok(p) => p.fetch_supported_models().await.unwrap_or_default(),
        Err(_) => Vec::new(),
    };

    let current = goose::agents::local_fusion::sidekick_model_uid().unwrap_or_default();
    let model: String = if models.is_empty() {
        cliclack::input("Sidekick model")
            .default_input(&current)
            .required(true)
            .interact()?
    } else {
        let mut select = cliclack::select(format!("Sidekick model ({provider})")).filter_mode();
        for m in &models {
            select = select.item(m.clone(), m.clone(), "");
        }
        select.interact()?
    };

    config.set_param("KARMX_SIDEKICK_PROVIDER", &provider)?;
    config.set_param("KARMX_SIDEKICK_MODEL", &model)?;
    let _ = cliclack::log::success(format!("Sidekick runs on {provider} / {model}"));
    print_config_file_saved()?;
    Ok(())
}

/// Configure extensions that can be used with karmx
...
pub fn configure_extensions_dialog() -> anyhow::Result<()> {
    let extension_type = cliclack::select("What type of extension would you like to add?")
        .item(
            "built-in",
            "Built-in Extension",
            "Use an extension that comes with karmx",
        )
        .item(
            "stdio",
...
pub async fn configure_settings_dialog() -> anyhow::Result<()> {
    ...
    let setting_type = setting_select
        ...
        .item(
            "recipe",
            "karmx recipe github repo",
            "karmx will pull recipes from this repo if not found locally.",
        )
        .interact()?;
...
pub fn configure_goose_mode_dialog() -> anyhow::Result<()> {
    ...
    if goose::config::env_var("GOOSE_MODE").is_ok() {
        let _ = cliclack::log::info(
            "Notice: KARMX_MODE environment variable is set and will override the configuration here.",
        );
    }

    let mode = cliclack::select("Which karmx mode would you like to configure?")
        .item(
            GooseMode::Auto,
            "Auto Mode",
...
pub fn configure_telemetry_dialog() -> anyhow::Result<()> {
    ...
    if goose::config::env_var("GOOSE_TELEMETRY_OFF").is_ok() {
        let _ = cliclack::log::info(
            "Notice: KARMX_TELEMETRY_OFF environment variable is set and will override the configuration here.",
        );
    }
    ...
    if enabled {
        cliclack::outro("Telemetry enabled - thank you for helping improve karmX!")?;
    } else {
...
pub fn configure_tool_output_dialog() -> anyhow::Result<()> {
    ...
    if goose::config::env_var("GOOSE_CLI_MIN_PRIORITY").is_ok() {
        let _ = cliclack::log::info(
            "Notice: KARMX_CLI_MIN_PRIORITY environment variable is set and will override the configuration here.",
        );
    }
...
pub fn configure_keyring_dialog() -> anyhow::Result<()> {
    ...
    if goose::config::env_var("GOOSE_DISABLE_KEYRING").is_ok() {
        let _ = cliclack::log::info(
            "Notice: KARMX_DISABLE_KEYRING environment variable is set and will override the configuration here.",
        );
    }
    ...
    match storage_option {
        "keyring" => {
            ...
            let _ =
                cliclack::log::info("You may need to restart karmx for this change to take effect");
        }
        "file" => {
...
/// Configure experiment features that can be used with karmx
...
fn configure_recipe_dialog() -> anyhow::Result<()> {
    ...
    let config = Config::global();
    let default_recipe_repo = goose::config::env_var(key_name)
        .ok()
        .or_else(|| config.get_param(key_name).unwrap_or(None));
    let mut recipe_repo_input = cliclack::input(
        "Enter your karmx recipe GitHub repo (owner/repo): eg: my_org/karmx-recipes",
    )
    .required(false);
...
pub fn configure_max_turns_dialog() -> anyhow::Result<()> {
    ...
    cliclack::outro(format!(
        "Set maximum turns to {} - karmx will ask for input after {} consecutive actions",
        max_turns, max_turns
    ))?;
...
fn add_provider() -> anyhow::Result<()> {
    ...
    if requires_auth {
        let auth_mode = cliclack::select("How should karmx obtain credentials for this provider?")
