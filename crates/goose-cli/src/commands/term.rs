...
alias @karmx='{goose_bin} term run'
alias @kx='{goose_bin} term run'

karmx_preexec() {
    [[ "$1" =~ ^karmx\ term ]] && return
    [[ "$1" =~ ^(@karmx|@kx)($|[[:space:]]) ]] && return
    ('{goose_bin}' term log "$1" &) 2>/dev/null
}

if [[ -z "$karmx_preexec_installed" ]]; then
    karmx_preexec_installed=1
    trap 'karmx_preexec "$BASH_COMMAND"' DEBUG
fi{command_not_found_handler}"#,
    command_not_found: Some(
        r#"

command_not_found_handle() {
    echo "Command '$1' not found. Asking karmx..."
    '{goose_bin}' term run "$@"
    return 0
}"#,
    ),
...
add-zsh-hook preexec karmx_preexec{command_not_found_handler}"#,
    command_not_found: Some(
        r#"
...
function @karmx; {goose_bin} term run $argv; end
function @kx; {goose_bin} term run $argv; end

function karmx_preexec --on-event fish_preexec
    string match -q -r '^karmx term' -- $argv[1]; and return
    string match -q -r '^(@karmx|@kx)($|\s)' -- $argv[1]; and return
...
def --wrapped @karmx [...args] { run-external "{goose_bin}" "term" "run" ...$args }
def --wrapped @kx [...args] { run-external "{goose_bin}" "term" "run" ...$args }

if (($env | get -o KARMX_NU_PREEXEC_INSTALLED | default false) != true) {
    $env.KARMX_NU_PREEXEC_INSTALLED = true
...
$env.config.hooks.command_not_found = {|command_name|
    ...
    print $"Command '($command_name)' not found. Asking karmx..."
...
static POWERSHELL_CONFIG: ShellConfig = ShellConfig {
    script_template: r#"$env:AGENT_SESSION_ID = "{session_id}"
function @karmx {{ & '{goose_bin}' term run @args }}
function @kx {{ & '{goose_bin}' term run @args }}

Set-PSReadLineKeyHandler -Chord Enter -ScriptBlock {{
    $line = $null
    [Microsoft.PowerShell.PSConsoleReadLine]::GetBufferState([ref]$line, [ref]$null)
    if ($line -notmatch '^karmx term' -and $line -notmatch '^(@karmx|@kx)($|\s)') {{
...
pub async fn handle_term_init(
    ...
    let goose_bin = std::env::current_exe()
        ...
        .unwrap_or_else(|_| "karmx".to_string());
...
pub async fn handle_term_log(command: String) -> Result<()> {
    let session_id = std::env::var("AGENT_SESSION_ID").map_err(|_| {
        anyhow!(
            "AGENT_SESSION_ID not set. Initialize terminal integration with `karmx term init <shell>` and reload your shell first."
        )
    })?;
    ...
    let session_id = std::env::var("AGENT_SESSION_ID").map_err(|_| {
        anyhow!(
            "AGENT_SESSION_ID not set.\n\n\
             Initialize terminal integration with `karmx term init <shell>` in your shell profile, \
...
/// Handle `karmx term info` - print compact session info for prompt integration
pub async fn handle_term_info() -> Result<()> {
    ...
    let model_name = session
        ...
        .map(|name| {
            ...
            if let Some(stripped) = short
                .strip_prefix("karmx-")
                .or_else(|| short.strip_prefix("goose-"))
            {
                stripped.to_string()
            } else {
                short.to_string()
...
mod tests {
    ...
    fn render_term_init_script_includes_nushell_hooks() {
        let script = render_term_init_script(Shell::Nu, "session-123", "/tmp/karmx", false);

        assert!(script.contains("$env.AGENT_SESSION_ID = \"session-123\""));
        assert!(script.contains("def --wrapped @karmx [...args]"));
        assert!(script.contains("def --wrapped @kx [...args]"));
        assert!(script.contains("KARMX_NU_PREEXEC_INSTALLED"));
        assert!(script.contains("$env.config.hooks.pre_execution"));
        assert!(script.contains("job spawn { run-external \"/tmp/karmx\" \"term\" \"log\" $line | complete | ignore } | ignore"));
        assert!(!script.contains("command_not_found = {|command_name|"));
    }

    #[test]
    fn render_term_init_script_includes_nushell_default_handler() {
        let script = render_term_init_script(Shell::Nu, "session-123", "/tmp/karmx", true);

        assert!(script.contains("$env.config.hooks.command_not_found = {|command_name|"));
        assert!(script
            .contains("run-external \"/tmp/karmx\" \"term\" \"run\" $prompt | complete | ignore"));
    }

    #[test]
    fn render_term_init_script_skips_unsupported_default_handler() {
        let script = render_term_init_script(Shell::Fish, "session-123", "/tmp/karmx", true);
