//! `karmx roam` — peer-to-peer agent access over iroh.
//!
...
//! accepted peer gets karmx's full ACP surface.
//!
//! Subcommands:
...
const CARD_SCHEME: &str = "karmx+roam://";
const LEGACY_CARD_SCHEME: &str = "goose+roam://";
...
/// Config key (or `KARMX_ROAM_RELAYS`/`GOOSE_ROAM_RELAYS` env) overriding the
/// relay URLs the roaming endpoint uses. When unset, roaming uses the managed
/// relays below — never iroh's shared public n0 relays.
...
///
/// Override with `KARMX_ROAM_RELAYS` to point at other relays (e.g. a private
/// deployment).
...
///
/// Uses `KARMX_ROAM_RELAYS`/`GOOSE_ROAM_RELAYS` (env or config file) when set —
/// optionally authenticated with the relay-token secret applied to each —
/// otherwise the default managed relays. Never iroh's public n0 relays.
///
/// Fails when the relays key is set but unreadable: a deployment that
...
pub enum RoamCommand {
    ...
    /// Each connected peer gets karmx's full ACP surface — it drives its own
    ...
    Connect {
        /// A saved peer nickname (see `roam peers`) or a `karmx+roam://...` card.
        target: String,
    ...
    ///
    /// Defaults to stdio (for a client that spawns `karmx roam bridge ...` as a
...
pub enum PeersCommand {
    ...
    Add {
        /// The peer's `karmx+roam://...` card.
        card: String,
        /// Friendly nickname (defaults to a short id if omitted).
        name: Option<String>,
    },
    /// Accept inbound connections from a peer's key. The target is a saved
    /// nickname or a `karmx+roam://...` card (which is also saved to the address
    /// book). An accepted peer gets karmx's full ACP surface.
    Accept {
        /// A saved nickname or a `karmx+roam://...` card.
        target: String,
...
async fn handle_id(qr: bool) -> Result<()> {
    ...
    eprintln!();
    eprintln!("the peer adds it with:  karmx roam peers add '<card>' <name>");
    eprintln!("and accepts you with:   karmx roam peers accept <name>");
    node.shutdown().await?;
    Ok(())
}
...
async fn handle_pair(name: Option<String>) -> Result<()> {
    ...
    let decoded = ConnectionCard::decode(device_card)
        .context("that does not look like a karmx+roam:// connection card")?;
    let name =
...
async fn handle_peers(command: PeersCommand) -> Result<()> {
    ...
    match command {
        PeersCommand::Add { card, name } => {
            ...
            eprintln!("accept connections from it with: karmx roam peers accept {name}");
            Ok(())
        }
        PeersCommand::Accept { target, name } => {
            ...
            eprintln!("a running `karmx roam share` picks this up on the next connection");
            Ok(())
        }
        ...
        PeersCommand::List => {
            ...
            if peers.is_empty() && accepted.is_empty() {
                eprintln!("no saved peers; add one with `karmx roam peers add '<card>' <name>`");
                return Ok(());
            }
...
/// advisory lock decides ownership across all karmx processes (desktop
...
pub(crate) fn try_acquire_roam_lock() -> Result<std::fs::File> {
    try_acquire_roam_lock_owner()?.ok_or_else(|| {
        anyhow::anyhow!("another karmx process is already running the roaming endpoint")
    })
}
    ...
    if accepted_count == 0 {
        eprintln!(
            "warning: no peers are accepted yet — no one can connect.\n\
             Accept a peer's key first: karmx roam peers accept <name|card>"
        );
    }
...
fn resolve_card(target: &str) -> Result<ConnectionCard> {
    if target.starts_with(CARD_SCHEME) || target.starts_with(LEGACY_CARD_SCHEME) {
    ...
    match book.get(target) {
        ...
        None => anyhow::bail!(
            "no saved peer named `{target}` (and it is not a card); see `karmx roam peers`"
        ),
    }
}
