//! The roaming host adapter: serve karmx's **full** ACP surface to each
//! connecting peer.
//!
//! Roaming is just an authenticated p2p ACP transport. This adapter is the thin
//! seam that hands an authorized iroh stream straight to karmx's real
...
/// An [`AcpStreamServer`] that serves karmx's full ACP surface, a fresh agent
/// per connection.
pub struct FullAcpBridge {
    ...
    /// `AcpServer` (e.g. `karmx serve`) leaves `session_cwd` unset for its
    /// local clients.
