...
thread_local! {
    static CURRENT_THEME: RefCell<Theme> = RefCell::new(
        goose::config::env_var("GOOSE_CLI_THEME").ok()
...
/// Opt-in via `KARMX_CLI_BELL=true` (environment or config); terminals
        ...
        match failure.label.as_deref() {
            ...
            Some(label) => {
                ...
                eprintln!(
                    "{}",
                    style(format!(
                        "    Hint: ask karmx to help debug the '{}' extension",
                        label
                    ))
                    .dim()
                );
...
pub fn display_session_info(
    ...
    // karmX wordmark with session info on the right
    println!();
    println!(
        "  {}  {} {} {} {} {}",
        style("█ █ ▄▀█ █▀▄ █▄▀▄█ ╳").white(),
    ...
    println!("  {}", style("     karmx is ready").white());
}
...
fn set_terminal_title() {
    ...
    print!("\x1b]0;karmX {}\x07", sanitized);
    let _ = std::io::stdout().flush();
}
...
pub fn display_context_usage(total_tokens: usize, context_limit: usize, sidekick: Option<String>) {
    use console::style;
...
mod tests {
    ...
    fn terminal_line_sanitizer_preserves_plain_unicode_text() {
        assert_eq!(sanitize_terminal_line("karmx ◆\t日本語"), "karmx ◆\t日本語");
    }
