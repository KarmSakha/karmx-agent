...
async fn resolve_provider_and_model(
    ...
    let provider_name = session_config
        ...
        .unwrap_or_else(|| {
            output::render_error("No provider configured. Run 'karmx configure' first.");
            process::exit(1);
        });
    ...
    let matching_environment_model =
        if provider_overridden && configured_provider.as_deref() == Some(provider_name.as_str()) {
            goose::config::env_var("GOOSE_MODEL").ok()
        } else {
            None
        };
    ...
    let model_name = session_config
        ...
        .unwrap_or_else(|| {
            output::render_error("No model configured. Run 'karmx configure' first.");
            process::exit(1);
        });
                    ...
                    && is_provider_unavailable_error(&e) =>
            {
                ...
                match create(&fallback_provider, extensions_for_provider.clone()).await {
                    ...
                    Err(e2) => {
                        output::render_error(&format!(
                        "Error {}.\n\
                        Please check your system keychain and run 'karmx configure' again.\n\
                        If your system is unable to use the keyring, set KARMX_DISABLE_KEYRING=true to use file-based secrets instead.",
                        e2
                    ));
                        process::exit(1);
                    }
