...
pub struct CliSession {
    ...
    /// Draft restored into the next prompt line after Ctrl+P enhancement.
    pending_draft: Option<String>,
    /// Lazy workspace index for KARMX_CONTEXT_AUTO_RETRIEVE; built once per
    /// session so per-message retrieval stays cheap.
    retrieval_index: Option<
        std::sync::Arc<
            goose::agents::platform_extensions::context_engine::workspace::WorkspaceIndex,
        >,
    >,
}
...
pub struct CompletionCache {
    ...
    pub current_session_model: String,
...
impl CompletionCache {
    fn new() -> Self {
        Self {
            ...
            current_session_model: String::new(),
...
impl CliSession {
    ...
    async fn run_interactive(&mut self, prompt: Option<String>) -> Result<()> {
        ...
        loop {
            ...
            let initial = self.pending_draft.take();
            let input =
                input::get_input(&mut editor, Some(&conversation_strings), initial.as_deref())?;
    ...
    async fn handle_input(
        ...
        match input {
                    ...
                    None => {
                        output::render_error(
                            "No editor found. Set one with:\n  \
                                 karmx configure set goose_prompt_editor \"vim\"\n  \
            ...
            InputResult::ConfigureSidekick => {
                if let Err(e) = crate::commands::configure::configure_sidekick_dialog().await {
                    output::render_error(&format!("Sidekick configuration failed: {e}"));
                }
            }
            InputResult::EnhancePrompt(draft) => {
                self.enhance_draft(&draft, conversation_messages).await;
            }
        }
        Ok(())
    }

    /// Ctrl+P: rewrite the draft via the context engine's enhance flow with
    /// recent conversation + cwd as context, then restore it into the next
    /// prompt line for editing. Fail-open: the original draft comes back on
    /// any error so nothing typed is lost.
    async fn enhance_draft(&mut self, draft: &str, conversation: &[String]) {
        let provider = match self.agent.provider().await {
            Ok(p) => p,
            Err(e) => {
                output::render_error(&format!("enhance unavailable: {e}"));
                self.pending_draft = Some(draft.to_string());
                return;
            }
        };
        ...
        for line in conversation.iter().rev().take(4) {
            let line: String = line.chars().take(500).collect();
            context.push_str("\n---\n");
            context.push_str(&line);
        }

        println!("{}", console::style("  enhancing prompt…").dim());
        let e = goose::agents::platform_extensions::context_engine::enhance::enhance_with_context(
            &provider,
            &model_config,
            draft,
            &context,
        ...
        if e.was_enhanced {
            println!(
                "{}",
                console::style("  ✓ prompt enhanced — edit and send, or Ctrl+C to clear").cyan()
            );
            self.pending_draft = Some(e.prompt);
        } else {
            let detail = e
                .error_message
                .unwrap_or_else(|| e.status.as_str().to_string());
            output::render_error(&format!("enhance kept original draft ({detail})"));
            self.pending_draft = Some(draft.to_string());
        }
    }

    /// When KARMX_CONTEXT_AUTO_RETRIEVE is set, run a lexical
    /// `codebase_retrieval`-style search on the outgoing text and append the
    /// hits as a context block, so the engine fires every turn instead of
    /// only when the model decides to call the tool.
    fn with_auto_retrieval(&mut self, content: &str) -> String {
        let enabled = Config::global()
            .get_param::<bool>("KARMX_CONTEXT_AUTO_RETRIEVE")
            .unwrap_or(false);
        if !enabled {
            return content.to_string();
        }
        if self.retrieval_index.is_none() {
            let cwd = std::env::current_dir().unwrap_or_default();
            self.retrieval_index =
                goose::agents::platform_extensions::context_engine::workspace::WorkspaceIndex::build(
                    &cwd, 50_000,
                )
                .ok()
                .map(std::sync::Arc::new);
        }
        let Some(index) = &self.retrieval_index else {
            return content.to_string();
        };
        let hits = index.search(content, 20);
        if hits.is_empty() {
            return content.to_string();
        }
        format!(
            "{content}\n\n<context_engine codebase_retrieval>\n{}\n</context_engine>",
    ...
    async fn handle_message_input(
        ...
        match self.run_mode {
            RunMode::Normal => {
                history.save(editor);
                let content = self.with_auto_retrieval(content);
                let (text, images) = input::expand_mentions(&content);
    ...
    async fn handle_model(&mut self, options: input::ModelCommandOptions) -> Result<()> {
        ...
        if let Some(effort) = options.thinking {
            new_model_config = new_model_config.with_thinking_effort(effort);
        }
            ...
            Err(e) => {
                output::render_error(&format!(
                    "Cannot switch to provider '{}': {}\n\
                         Set credentials via `karmx configure` or the appropriate environment variable.\n\
        ...
        if let Some(effort) = options.thinking {
            let _ = Config::global().set_goose_thinking_effort(effort);
        }
        ...
        if provider_unchanged {
            output::goose_mode_message(&format!(
                ...
                current_model_name, target_model_name, current_provider_name, effort_note
            ));
        } else {
            output::goose_mode_message(&format!(
    ...
    async fn refresh_completion_cache(
        ...
        cache.current_session_model = session_model;
    ...
    pub async fn display_context_usage(&self) -> Result<()> {
        ...
        let sidekick = goose::agents::local_fusion::sidekick_provider()
            .zip(goose::agents::local_fusion::sidekick_model_uid())
            .map(|(p, m)| format!("{p}/{m}"));
        ...
        match self.get_session().await {
            Ok(metadata) => {
                ...
                output::display_context_usage(total_tokens, context_limit, sidekick.clone());
            ...
            Err(_) => {
                output::display_context_usage(0, context_limit, sidekick);
            }
        }
...
async fn get_reasoner(
    ...
    let provider = if let Ok(provider) = config.get_param::<String>("GOOSE_PLANNER_PROVIDER") {
        ...
        println!("WARNING: KARMX_PLANNER_PROVIDER not found. Using default provider...");
        config
            .get_goose_provider()
            .expect("No provider configured. Run 'karmx configure' first")
    };
    ...
    let model = if let Ok(model) = config.get_param::<String>("GOOSE_PLANNER_MODEL") {
        ...
        println!("WARNING: KARMX_PLANNER_MODEL not found. Using default model...");
        config
            .get_goose_model()
            .expect("No model configured. Run 'karmx configure' first")
    };
