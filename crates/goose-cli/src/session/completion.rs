...
impl GooseCompleter {
    ...
    fn complete_file_path(&self, line: &str, ctx: &Context) -> Result<(usize, Vec<Pair>)> {
        ...
        if let Some(last_part) = parts.last() {
            ...
            // `@path` mentions complete as file paths after the marker.
            let (token, at_offset) = match last_part.strip_prefix('@') {
                Some(rest) => (rest, 1),
                None => (*last_part, 0),
            };
            ...
            let pos = line.len() - last_part.len();
            let (start, candidates) = self.filename_completer.complete(token, token.len(), ctx)?;

            // Return the completion results, with adjusted position
            return Ok((pos + at_offset + start, candidates));
        }
    ...
    fn hint(&self, line: &str, _pos: usize, _ctx: &Context<'_>) -> Option<Self::Hint> {
        ...
        match cache.hint_status {
            HintStatus::Interrupted => {
                Some("Interrupted, what should karmx work on instead?".to_string())
            }
            ...
            HintStatus::Default => {
                ...
                Some(format!(
                    "Enter to send · Ctrl+{newline_key} newline · Ctrl+P enhance"
                ))
            }
        }
    }
