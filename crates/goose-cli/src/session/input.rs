...
pub enum InputResult {
    ...
    ConfigureSidekick,
    /// Ctrl+P submitted the draft for context-aware enhancement.
...
/// Ctrl+P captures the current buffer and submits it for enhancement. The
/// handler stores the draft so get_input can return EnhancePrompt; AcceptLine
/// delivers the raw buffer as the readline result either way.
struct EnhanceDraftHandler {
    slot: Arc<std::sync::RwLock<Option<String>>>,
}

impl rustyline::ConditionalEventHandler for EnhanceDraftHandler {
    fn handle(
        &self,
        _event: &rustyline::Event,
        _n: u16,
...
/// Ctrl+V pastes a clipboard image: saves it under the state dir and inserts
/// an `@path` mention into the buffer. Returns None when the clipboard holds
/// no image, so ordinary text paste keeps working through bracketed paste.
struct ClipboardImageHandler;

impl rustyline::ConditionalEventHandler for ClipboardImageHandler {
    fn handle(
        &self,
        _event: &rustyline::Event,
        _n: u16,
        ...
        let dir = goose::config::paths::Paths::in_state_dir("clipboard");
        ...
        Some(rustyline::Cmd::Insert(1, format!("@{}", path.display())))
    }
}

/// The Ctrl-modified character that inserts a newline instead of submitting the
/// prompt. Configurable via `KARMX_CLI_NEWLINE_KEY`, defaulting to `j` (Ctrl+J).
...
const MAX_MENTION_CHARS: usize = 32_000;
const MENTION_IMAGE_EXTS: &[&str] = &["png", "jpg", "jpeg", "gif", "webp", "bmp"];

fn mention_mime(ext: &str) -> &'static str {
    match ext {
        "png" => "image/png",
        "jpg" | "jpeg" => "image/jpeg",
        "gif" => "image/gif",
...
/// Expand `@path` mentions in an outgoing message. Resolvable files attach:
/// images become image content parts (base64 + mime), text files append as
/// fenced `<file>` blocks, and the `@token` is replaced by a short marker.
/// Unresolvable tokens pass through so `@handle`/`user@host` stay literal.
pub(super) fn expand_mentions(content: &str) -> (String, Vec<(String, String)>) {
    ...
    for chunk in content.split_inclusive(char::is_whitespace) {
        ...
        let ws = &chunk[word.len()..];
        let mut replaced = false;

        if attached < MAX_MENTIONS {
                ...
                if !path_str.is_empty() {
                    ...
                    if path.is_file() {
                        ...
                        let ext = path
                            ...
                            .unwrap_or("")
                            .to_lowercase();
                        if MENTION_IMAGE_EXTS.contains(&ext.as_str()) {
                            if let Ok(bytes) = std::fs::read(&path) {
                                use base64::Engine;
                                images.push((
                                    base64::engine::general_purpose::STANDARD.encode(&bytes),
                                    mention_mime(&ext).to_string(),
                                ));
                                text.push_str(&format!("[image: {name}]"));
                                replaced = true;
                            }
                        } else if let Ok(raw_text) = std::fs::read_to_string(&path) {
                            let clipped: String =
                                raw_text.chars().take(MAX_MENTION_CHARS).collect();
...
pub fn get_input(
    ...
    let enhance_slot = Arc::new(std::sync::RwLock::new(None::<String>));
    editor.bind_sequence(
        rustyline::KeyEvent(rustyline::KeyCode::Char('p'), rustyline::Modifiers::CTRL),
        rustyline::EventHandler::Conditional(Box::new(EnhanceDraftHandler {
            slot: enhance_slot.clone(),
        })),
    );

    editor.bind_sequence(
        rustyline::KeyEvent(rustyline::KeyCode::Char('v'), rustyline::Modifiers::CTRL),
        rustyline::EventHandler::Conditional(Box::new(ClipboardImageHandler)),
    );

    let input = match read_paste_aware_input(editor, paste_state, initial) {
        Ok(text) => text,
        Err(e) => match e {
    ...
    if let Some(draft) = enhance_slot.write().ok().and_then(|mut slot| slot.take()) {
        return Ok(InputResult::EnhancePrompt(draft));
    }
    ...
    match handle_slash_command(&input) {
        // Bare `/model` opens the model picker when cached models exist;
        // cancelling falls back to the usual current-model display.
        Some(InputResult::Model(options))
            if options.provider.is_none() && options.model.is_none() =>
        {
            Ok(super::slash_menu::pick_model(&completion_cache)
                .unwrap_or(InputResult::Model(options)))
        }
        // Bare `/model --provider` opens the provider picker.
        Some(InputResult::Model(options))
            if options.provider.as_deref() == Some("") && options.model.is_none() =>
        {
            Ok(super::slash_menu::pick_provider(&completion_cache)
                .unwrap_or(InputResult::Model(options)))
        }
        ...
        None => {
            let cwd = std::env::current_dir().unwrap_or_default();
            let known =
                goose::agents::execute_commands::is_known_slash_command(input.trim(), Some(&cwd));
            if known || !super::slash_menu::should_open(&input) || !std::io::stdout().is_terminal()
            {
                return Ok(InputResult::Message(input.trim().to_string()));
            }
            Ok(super::slash_menu::run(&input, &completion_cache).unwrap_or(InputResult::Retry))
        }
    }
}

pub(super) fn handle_slash_command(input: &str) -> Option<InputResult> {
    ...
    match input {
        ...
        "/sidekick" => Some(InputResult::ConfigureSidekick),
...
/mode <name> - Set the karmx mode to use ({modes})
/model - Open the model picker (↑↓ select, → thinking effort, tab sidekick)
/model [name] - Switch models for this session while keeping the same provider
/model --provider <name> [model] - Switch to a different provider (optionally specifying a model)
/plan <message_text> -  Enters 'plan' mode with optional message. Create a plan based on the current messages and asks user if they want to act on it.
                        If user acts on the plan, karmx mode is set to 'auto' and returns to 'normal' karmx mode.
                        To warm up karmx before using '/plan', we recommend setting '/mode approve' & putting appropriate context into karmx.
                        The model is used based on $KARMX_PLANNER_PROVIDER and $KARMX_PLANNER_MODEL environment variables.
                        If no model is set, the default model is used.
/endplan - Exit plan mode and return to 'normal' karmx mode.
...
/edit [text] - Open your prompt editor to compose a message. Optionally pre-fill with text.
               Uses $KARMX_PROMPT_EDITOR, $VISUAL, or $EDITOR (in that order).
/skills - List available skills or enable skills by name (usage: /skills [<name>...])
/sidekick - Pick the provider and model the sidekick (fusion) runs on
/fusion - Pick a sidekick model, then the lead model
...
Navigation:
Enter - Send message
Ctrl+{newline_key} - Add a newline (configurable via KARMX_CLI_NEWLINE_KEY)
Ctrl+C - Clear current line if text is entered, otherwise exit the session
Up/Down arrows - Navigate through command history
KARMX_CLI_BELL=true - Ring the terminal bell when karmx finishes a turn or needs approval"
    )
}
...
fn print_editor_help() {
  ...
  Configure editor: karmx configure set goose_prompt_editor \"vim\"
  ...
  To use inline prompts with on-demand /edit: karmx configure set goose_prompt_editor_always false"
    );
}
...
mod tests {
    ...
    fn test_handle_slash_command() {
        ...
        if let Some(InputResult::Model(ModelCommandOptions {
            provider, model, ..
