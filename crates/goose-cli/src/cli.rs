...
const SERVER_SECRET_KEY_ENV: &str = "GOOSE_SERVER__SECRET_KEY";
...
/// karmX — an AI agent for your terminal with browser preview, a context
/// engine, Chrome computer control, and local sidekick execution.
#[derive(Parser)]
#[command(name = "karmx", author, version, display_name = "", about, long_about = None)]
pub struct Cli {
    #[command(subcommand)]
...
pub struct SessionOptions {
    ...
    #[arg(
        ...
        long_help = "Run extensions (stdio and built-in) inside the specified container. The extension must exist in the container. For built-in extensions, karmx must be installed inside the container."
    )]
...
pub struct ExtensionOptions {
    ...
    #[arg(
        ...
        long_help = "Add one or more builtin extensions that are bundled with karmx by specifying their names, comma-separated",
        value_delimiter = ','
    )]
...
pub struct InputOptions {
    ...
    #[arg(
        ...
        long = "text",
        value_name = "TEXT",
        help = "Input text to provide to karmx directly",
        long_help = "Input text containing commands for karmx. Use this in lieu of the instructions argument.",
    ...
    #[arg(
        ...
        help = "Dynamic parameters (e.g., --params username=alice --params channel_name=my-channel)",
...
pub struct ModelOptions {
    ...
    #[arg(
        ...
        long_help = "Override the KARMX_PROVIDER environment variable for this run. Available providers include openai, anthropic, ollama, databricks, gemini-cli, claude-code, and others."
    )]
    ...
    #[arg(
        ...
        long_help = "Override the KARMX_MODEL environment variable for this run. The model must be supported by the specified provider."
    )]
...
enum SessionCommand {
    ...
    Import {
        #[arg(
            help = "Path to a karmx session export, a Claude Code, Codex, or Pi .jsonl transcript, or a goose://sessions/nostr share link"
        )]
        input: String,
...
#[derive(Subcommand)]
enum SkillsCommand {
    /// List all skills available to the karmx agent
    #[command(about = "List all skills available to the karmx agent")]
    List,
}
...
#[derive(Subcommand)]
enum Command {
    /// Configure karmx settings
    #[command(about = "Configure karmx settings")]
    Configure {},

    /// Display karmx configuration information
    #[command(about = "Display karmx information")]
    Info {
    ...
    #[command(about = "Check that your karmX setup is working")]
    Doctor {},

    /// Manage system prompts and behaviors
    #[command(about = "Run one of the mcp servers bundled with karmx")]
    Mcp {
        #[arg(value_parser = clap::value_parser!(McpCommand))]
        server: McpCommand,
    },

    /// Run karmx as an ACP (Agent Client Protocol) agent
    #[command(about = "Run karmx as an ACP agent server on stdio")]
    Acp {
    ...
    Serve {
        ...
        #[arg(
            ...
            help = "Start the ACP endpoint without requiring KARMX_SERVER__SECRET_KEY"
        )]
        ...
        /// Also expose this server over karmx roam (p2p) so paired devices can connect remotely
        #[cfg(feature = "roaming")]
        #[arg(
            long,
            help = "Also expose this server over karmx roam (p2p) so paired devices can connect remotely"
        )]
        roam: bool,
    },
    ...
    /// Rebuild and update the karmx CLI from source
    #[cfg(feature = "update")]
    #[command(about = "Rebuild and update the karmx CLI from source")]
    Update {
        /// Allow building from a working tree with uncommitted changes
        #[arg(
            short,
            long,
            help = "Allow building from a working tree with uncommitted changes",
            long_help = "Build a canary binary from the current checkout even if it has uncommitted changes. By default, update refuses to build a dirty working tree."
        )]
        canary: bool,

        /// Enforce to re-configure karmx during update
        #[arg(short, long, help = "Enforce to re-configure karmx during update")]
        reconfigure: bool,
    },

    /// Terminal-integrated session (one session per terminal)
    #[command(
        about = "Terminal-integrated karmx session",
        long_about = "Runs a karmx session tied to your terminal window.\n\
                      Each terminal maintains its own persistent session that resumes automatically.\n\n\
                      Setup:\n  \
                        eval \"$(karmx term init zsh)\"  # zsh/bash\n  \
                        let init = ($nu.cache-dir | path join \"karmx-term-init.nu\"); ^karmx term init nu | save --force $init; source $init\n\n\
                      Usage:\n  \
                        karmx term run \"list files in this directory\"\n  \
                        @karmx \"create a python script\"  # using alias\n  \
                        @kx \"quick question\"  # short alias"
    )]
    Term {
        #[command(subcommand)]
        command: TermCommand,
    ...
    Completion {
        ...
        #[arg(long, default_value = "karmx", help = "Provide a custom binary name")]
        bin_name: String,
    },
    ...
    /// runs the review through karmx.
    #[command(about = "Review the current diff using karmx")]
    Review {
        ...
        /// orchestrator dispatches one `karmx run` subprocess per check
...
enum TermCommand {
    ...
    #[command(
        ...
        long_about = "Prints shell configuration to set up terminal-integrated sessions.\n\
                      Each terminal gets a persistent karmx session that automatically resumes.\n\n\
                      Setup:\n  \
                        echo 'eval \"$(karmx term init zsh)\"' >> ~/.zshrc\n  \
                        source ~/.zshrc\n\n\
                        Nushell:\n  \
                        let init = ($nu.cache-dir | path join \"karmx-term-init.nu\")\n  \
                        ^karmx term init nu | save --force $init\n  \
                        source $init\n\n\
                      With --default (anything typed that isn't a command goes to karmx):\n  \
                        echo 'eval \"$(karmx term init zsh --default)\"' >> ~/.zshrc\n  \
                        ^karmx term init nu --default | save --force $init"
    )]
    Init {
        ...
        /// Make karmx the default handler for unknown commands
        #[arg(
            long = "default",
            help = "Make karmx the default handler for unknown commands",
            long_help = "When enabled, anything you type that isn't a valid command will be sent to karmx. Supported for zsh, bash, and nu."
        )]
        default: bool,
    },
    ...
    #[command(
        ...
        long_about = "Run a prompt in the terminal-integrated session.\n\n\
                      Examples:\n  \
                        karmx term run list files in this directory\n  \
                        @karmx list files  # using alias\n  \
                        @kx why did that fail  # short alias"
    )]
    Run {
        /// The prompt to send to karmx (multiple words allowed without quotes)
            ...
            match try_acquire_roam_lock_owner() {
                ...
                Ok(None) => {
                    if !standing_by {
                        standing_by = true;
                        eprintln!(
                            "another karmx process owns the roaming endpoint; standing by to take over if it exits"
                        );
                    }
                }
...
async fn start_roam_share(
    ...
    // Roaming sessions run where `karmx serve` was started: the connector's
...
async fn handle_serve_command(args: ServeCommandArgs) -> Result<()> {
    ...
    let env_secret = goose::config::env_var(SERVER_SECRET_KEY_ENV)
        .ok()
    ...
    if !require_token && !dangerously_unauthenticated {
        anyhow::bail!(
            "KARMX_SERVER__SECRET_KEY (or {SERVER_SECRET_KEY_ENV}) must be set to start `karmx serve`; pass --dangerously-unauthenticated to run without ACP authentication"
        );
    }
    if dangerously_unauthenticated && !require_token {
        warn!(
            "KARMX_SERVER__SECRET_KEY is not set and --dangerously-unauthenticated was passed; the ACP endpoint will accept unauthenticated connections"
        );
    }
    ...
    match (
        ...
        (Some(file), _, _) => {
            let contents = std::fs::read_to_string(file).unwrap_or_else(|err| {
                eprintln!(
                    "Instruction file not found — did you mean to use karmx run --text?\n{}",
                    err
                );
                std::process::exit(1);
            });
        ...
        LocalModelsCommand::Search {
            ...
            for model in &results {
                ...
                if let Some(variant) = recommended_variant(model, available_memory) {
                    ...
                    println!(
                        "    Download: karmx local-models download '{}'",
                        variant.download_id
                    );
                } else {
                    println!(
...
mod tests {
    ...
    fn completion_command_accepts_nushell_alias() {
        let cli = Cli::try_parse_from(["karmx", "completion", "nushell"]).expect("parse failed");
    ...
    fn session_accepts_provider_override_without_resume() {
        let cli = Cli::try_parse_from(["karmx", "session", "--provider", "openai"])
    ...
    fn session_accepts_model_override_without_resume() {
        let cli = Cli::try_parse_from(["karmx", "session", "--model", "gpt-5.4"])
    ...
    fn session_accepts_system_prompt() {
        let cli = Cli::try_parse_from(["karmx", "session", "--system", "extra instructions"])
    ...
    fn nushell_completion_generation_emits_module() {
        ...
        CompletionShell::Nu.generate(&mut cmd, "karmx", &mut buffer);

        let script = String::from_utf8(buffer).expect("utf8");
        assert!(script.contains("module completions"));
        assert!(script.contains("export extern karmx"));
    ...
    fn term_init_help_mentions_nushell() {
        ...
        assert!(help.contains("karmx term init nu"));
    ...
    fn skills_command_accepts_list_subcommand() {
        let cli = Cli::try_parse_from(["karmx", "skills", "list"]).expect("parse failed");
