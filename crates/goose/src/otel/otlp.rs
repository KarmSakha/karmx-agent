...
fn create_resource() -> Resource {
    ...
    let mut builder = Resource::builder_empty()
        .with_attributes([
            KeyValue::new("service.name", "karmx"),
            KeyValue::new("service.version", env!("CARGO_PKG_VERSION")),
            KeyValue::new("service.namespace", "karmx"),
...
fn create_otlp_tracing_layer() -> OtlpResult<OtlpTracingLayer> {
    ...
    let tracer = tracer_provider.tracer("karmx");
...
mod tests {
    ...
    fn test_create_resource_defaults() {
        ...
        assert_eq!(get("service.name").as_deref(), Some("karmx"));
        assert_eq!(
            get("service.version").as_deref(),
            Some(env!("CARGO_PKG_VERSION"))
        );
        assert_eq!(get("service.namespace").as_deref(), Some("karmx"));
    ...
    #[test_case(
        ...
        Resource::builder_empty()
            .with_attributes([KeyValue::new("service.name", "karmx"), KeyValue::new("service.version", env!("CARGO_PKG_VERSION")), KeyValue::new("service.namespace", "karmx"), KeyValue::new("host.name", session_host()), KeyValue::new("user.name", session_user())])
            .with_detector(Box::new(TelemetryResourceDetector))
            .build();
