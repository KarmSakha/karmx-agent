...
pub enum HookEvent {
    ...
    AfterShellExecution,
    Stop,
    /// Fired before each agent iteration (one model round-trip).
    ///
    /// Brackets every pass of the state-machine loop, which is finer-grained
    /// than [`HookEvent::Stop`] and is where per-turn budget and context checks
    /// naturally belong.
    PreAgentIteration,
    /// Fired after each agent iteration completes.
    PostAgentIteration,
}
...
impl HookEvent {
    fn name(&self) -> &'static str {
        match self {
            ...
            HookEvent::Stop => "Stop",
            HookEvent::PreAgentIteration => "PreAgentIteration",
            HookEvent::PostAgentIteration => "PostAgentIteration",
        }
    }
    ...
    fn from_name(name: &str) -> Option<Self> {
        Some(match name {
            ...
            "Stop" => HookEvent::Stop,
            "PreAgentIteration" => HookEvent::PreAgentIteration,
            "PostAgentIteration" => HookEvent::PostAgentIteration,
            _ => return None,
        })
    }
}
...
mod tests {
    ...
    /// Every event must survive name -> from_name, or a hook config naming it
    /// is silently ignored instead of matched.
    #[test]
    fn every_hook_event_round_trips_through_its_config_name() {
        let all = [
            ...
            HookEvent::Stop,
            HookEvent::PreAgentIteration,
            HookEvent::PostAgentIteration,
        ];
        for e in all {
            let name = e.name();
            assert_eq!(
                HookEvent::from_name(name),
                Some(e),
    ...
    #[test]
    fn iteration_hooks_are_distinct_from_stop() {
        assert_ne!(
            HookEvent::PreAgentIteration.name(),
            HookEvent::Stop.name(),
            "iteration hooks must not collide with the turn-level Stop"
        );
        assert_eq!(HookEvent::PreAgentIteration.name(), "PreAgentIteration");
        assert_eq!(HookEvent::PostAgentIteration.name(), "PostAgentIteration");
    }
    use super::*;
