...
/// Token state relevant to the compaction decision.
#[derive(Debug, Clone)]
pub struct CompactionState {
    pub phase: goose_context_management::Phase,
    pub current_tokens: usize,
    pub context_limit: usize,
    pub thresholds: goose_context_management::CompactionThresholds,
    /// Auto-compaction disabled: the fraction knob is outside `(0, 1)`.
    pub disabled: bool,
}

impl CompactionState {
    /// Whether the conversation should be compacted before this inference.
    ///
    /// True from `apply` onward, which is exactly the point the previous
    /// single-fraction check fired at — the thresholds default to that same
    /// boundary, so this is not a behaviour change on its own.
    pub fn needs_compaction(&self) -> bool {
        !self.disabled && self.current_tokens > self.thresholds.apply
    }

    /// Whether a background summary is worth starting now.
    pub fn should_start_summary(&self) -> bool {
        !self.disabled && self.phase.wants_summary()
    }
}

/// Resolve thresholds from config.
///
/// `GOOSE_COMPACTION_THRESHOLDS` (`<spawn>,<apply>,<hard>`) wins when set and
/// parseable. Otherwise thresholds are derived from the context limit and the
/// existing `GOOSE_AUTO_COMPACT_THRESHOLD` fraction, so behaviour without the
/// new knob is unchanged. A malformed triple falls back rather than failing the
/// turn — a bad config value must not break inference.
fn resolve_thresholds(
    context_limit: usize,
    fraction: f64,
) -> goose_context_management::CompactionThresholds {
    if let Ok(raw) = Config::global().get_param::<String>("GOOSE_COMPACTION_THRESHOLDS") {
        match goose_context_management::CompactionThresholds::parse(&raw) {
            Ok(t) => return t,
            Err(e) => warn!("ignoring invalid GOOSE_COMPACTION_THRESHOLDS {raw:?}: {e}"),
        }
    }
    goose_context_management::CompactionThresholds::from_context_limit(context_limit, fraction)
}

/// Evaluate the compaction decision for a conversation.
pub async fn evaluate_compaction(
    ...
    // A fraction outside (0,1) means auto-compaction is switched off. Keep that
    // meaning rather than reinterpreting it as a token count.
    let disabled = threshold <= 0.0 || threshold >= 1.0;
    let thresholds = resolve_thresholds(context_limit, threshold);

    Ok(CompactionState {
        phase: thresholds.phase(current_tokens),
        current_tokens,
        context_limit,
        thresholds,
        disabled,
...
/// Whether a summary should be produced before the next inference.
pub async fn check_if_compaction_needed(
    ...
    // A provider that manages its own context does its own compaction.
    if provider.manages_own_context() {
        return Ok(false);
    }
    Ok(
        evaluate_compaction(provider, conversation, threshold_override, session)
            .await?
            .needs_compaction(),
    )
}
...
mod compaction_state_tests {
    use super::*;
    use goose_context_management::{summary_is_fresh, CompactionThresholds, Phase};

    fn at(tokens: usize) -> CompactionState {
        let thresholds = CompactionThresholds::new(100, 200, 300).unwrap();
        CompactionState {
            phase: thresholds.phase(tokens),
    ...
    #[test]
    fn needs_compaction_only_once_past_apply() {
        assert!(!at(99).needs_compaction());
        assert!(
            !at(100).needs_compaction(),
            "spawning must not force a block"
        );
        assert!(
            !at(200).needs_compaction(),
            "apply is the boundary, not inclusive"
        );
        assert!(at(201).needs_compaction());
        assert!(at(300).needs_compaction());
    }

    #[test]
    fn a_summary_is_wanted_from_spawn_onward() {
        assert!(!at(99).should_start_summary());
        assert!(at(100).should_start_summary(), "spawn is the whole point");
        assert!(at(250).should_start_summary());
        assert!(at(400).should_start_summary());
    }

    #[test]
    fn the_spawn_window_does_not_block_but_still_prepares() {
        // the behaviour this feature exists for: work happens before it is needed
        let s = at(150);
        assert!(s.should_start_summary(), "should prepare");
        assert!(!s.needs_compaction(), "without holding up the turn");
        assert_eq!(s.phase, Phase::Spawning);
    }

    #[test]
    fn disabled_suppresses_both_paths() {
        let mut s = at(400);
        s.disabled = true;
        assert!(!s.needs_compaction(), "threshold outside (0,1) means off");
        assert!(!s.should_start_summary(), "and no background work either");
    }

    #[test]
    fn a_summary_is_dropped_once_history_outgrows_its_window() {
        // window = hard - apply = 100, mirroring compaction_stale_after
        let window = 100;
        assert!(summary_is_fresh(150, 200, window), "within the window");
        assert!(summary_is_fresh(150, 250, window), "exactly at the window");
        assert!(
            !summary_is_fresh(150, 251, window),
            "beyond it, applying would drop the growth"
        );
    }
}
