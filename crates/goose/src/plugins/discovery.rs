...
fn user_settings_path() -> Option<PathBuf> {
    let base = Paths::path_root().or_else(dirs::home_dir)?;
    let karmx_path = base.join(".config").join("karmx").join("settings.json");
    if karmx_path.exists() {
        return Some(karmx_path);
    }
    let goose_path = base.join(".config").join("goose").join("settings.json");
    Some(if goose_path.exists() {
        goose_path
    } else {
        karmx_path
    })
...
fn project_settings_path(project_root: &Path, local: bool) -> PathBuf {
    ...
    let karmx_path = project_root.join(".config").join("karmx").join(file);
    if karmx_path.exists() {
        return karmx_path;
    }
    let goose_path = project_root.join(".config").join("goose").join(file);
