...
pub fn build_deeplink(nevent: &str, decryption_key: &str) -> String {
    format!(
        "karmx://sessions/nostr?nevent={}&key={}",
...
pub fn parse_deeplink(deeplink: &str) -> Result<ParsedShareLink> {
    let parsed = url::Url::parse(deeplink).context("Invalid karmx session share link")?;
    let valid_scheme = matches!(parsed.scheme(), "karmx" | "goose");
    if !valid_scheme || parsed.host_str() != Some("sessions") || parsed.path() != "/nostr" {
        return Err(anyhow!("Invalid karmx Nostr session share link"));
    }
...
mod tests {
    ...
    async fn publish_builds_deeplink_and_encrypted_kind_30278_event() {
        ...
        assert!(share.deeplink.starts_with("karmx://sessions/nostr?"));
