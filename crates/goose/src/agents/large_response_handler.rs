...
pub fn process_tool_response(
    ...
    match response {
        Ok(mut result) => {
            ...
            // Collected separately rather than written inside the loop: the
            // loop consumes `result.content`, and mutating `result.meta` from
            // within it would fight that move for no reason.
            let mut overflowed: Option<String> = None;
                    ...
                    Some(text_content) => {
                        ...
                        if text_content.text.chars().count() > threshold {
                            ...
                            match write_large_text_to_file(&text_content.text) {
                                Ok(file_path) => {
                                    // Keep the original alongside the pointer.
                                    // It is host-side only - the format layer
                                    // serialises `tool_result.content` and
                                    // nothing else, so this does not re-inflate
                                    // the model's context, which is the whole
                                    // reason the text was moved out.
                                    overflowed = Some(text_content.text.clone());
            ...
            if let Some(original) = overflowed {
                let meta = result.meta.get_or_insert_with(Default::default);
                meta.0.insert(
                    goose_providers::conversation::tool_meta::OVERFLOW_CONTENT.to_string(),
...
mod tests {
    ...
    #[cfg(unix)]
    #[test]
    fn test_original_content_is_preserved_when_it_overflows() {
        // The pointer replaces the text in `content`, but the original must
        // remain retrievable rather than only reachable by parsing prose.
        let big = "x".repeat(max_tool_response_size() + 1);
        let response = CallToolResult::success(vec![ContentBlock::text(big.clone())]);
        let processed = process_tool_response(Ok(response)).expect("processed");

        let visible: String = processed
            .content
            .iter()
            ...
            .collect();
        assert!(
            visible.contains("stored in the file"),
            "content should be a pointer"
        );
        assert!(!visible.contains(&big), "the bulk should leave content");

        let preserved = processed
            .meta
            .as_ref()
            .and_then(|m| {
                m.0.get(goose_providers::conversation::tool_meta::OVERFLOW_CONTENT)
            })
            .and_then(|v| v.as_str())
            .expect("overflow content recorded");
        assert_eq!(preserved, big, "original must survive intact");
    }
