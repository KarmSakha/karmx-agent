...
struct SystemPromptContext {
    ...
    /// Whether the agent may be told to batch independent tool calls. Gated on
    /// the model, because a BYO endpoint may not handle batched calls at all.
...
impl<'a> SystemPromptBuilder<'a, PromptManager> {
    ...
    /// Tell the agent whether it may batch independent tool calls.
...
impl PromptManager {
    ...
    pub fn builder<'a>(&'a self) -> SystemPromptBuilder<'a, Self> {
        SystemPromptBuilder {
            ...
            goose_mode: None,
            // Conservative until the caller says otherwise: an unknown model is
            // not assumed to support batched tool calls.
