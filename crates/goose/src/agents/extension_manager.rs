...
const GOOSE_USER_AGENT: reqwest::header::HeaderValue =
    reqwest::header::HeaderValue::from_static(concat!("karmx/", env!("CARGO_PKG_VERSION")));
...
async fn create_unix_socket_http_client(
    ...
    custom_headers.insert(
        ...
        GOOSE_USER_AGENT
            ...
            .unwrap_or("karmx")
            .parse()
            .unwrap_or_else(|_| HeaderValue::from_static("karmx")),
    );
...
impl ExtensionManager {
    ...
    pub async fn add_extension(
        ...
        let effective_working_dir = working_dir
            .clone()
            .or_else(|| {
                crate::config::env_var("GOOSE_WORKING_DIR")
                    .ok()
                    .map(PathBuf::from)
            })
