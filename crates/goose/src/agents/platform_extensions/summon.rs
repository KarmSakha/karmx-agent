...
/// Parameters for the `sidekick` tool.
///
/// Deliberately smaller than `DelegateParams`: the sidekick is one named peer
/// the lead briefs repeatedly, not a general task launcher.
#[derive(Debug, Default, Deserialize)]
pub struct SidekickParams {
    /// The brief. Required.
    pub message: Option<String>,
    /// Wait for the report (default true). False parks the sidekick and lets
    /// the lead keep working.
    pub blocking: Option<bool>,
    /// Override the sidekick model for this call only.
    pub model: Option<String>,
}
...
pub fn discover_filesystem_sources(working_dir: &Path) -> Vec<SourceEntry> {
    ...
    let global_recipe_dirs: Vec<PathBuf> = crate::config::env_var("GOOSE_RECIPE_PATH")
        .ok()
        .into_iter()
        .flat_map(|p| {
...
impl SummonClient {
    ...
    fn create_sidekick_tool(&self) -> Tool {
        let schema = serde_json::json!({
            "type": "object",
            "properties": {
                "message": {
                    "type": "string",
                    "description": "The brief for the sidekick: context, the exact task, and the done-criteria. It works independently, so include what it needs to know."
                },
                "blocking": {
                    "type": "boolean",
                    "default": true,
                    "description": "Wait for the report (default: true). Set false to park the sidekick and keep working."
                },
                "model": {
                    "type": "string",
                    "description": "Override the sidekick model for this call. Defaults to the configured sidekick model (KARMX_SIDEKICK_MODEL / KARMX_SIDEKICK_PROVIDER)."
                }
            },
            "required": ["message"]
        });
        ...
        Tool::new(
            "sidekick",
            "Hand work to your sidekick: a persistent peer that works alongside you on the same machine (shared filesystem and repos; its shell sessions are separate from yours).\n\n\
             The user interacts with one agent: you. Do not mention the sidekick or distinguish its work from yours - describe the combined result in the first person as your own.\n\n\
             Delegate the hands-on work: exploration, implementation, environment setup, builds, linters, type-checks, test suites. Keep judgement and authority for yourself: planning and design, data analysis and measurement, eval harnesses, pipeline configuration, complex interactive browser work, reviewing the diff before it lands, and anything requiring user authority.\n\n\
             Brief it well: give context and done-criteria, then let it work. Name the narrowest verification commands that cover the change - it treats your list as mandatory, so \"run everything\" re-gates unchanged work on every handoff. Reserve a full-suite pass for one final gate.\n\n\
             Its report is a claim; the artifacts it references are the evidence. Review the diff and test output rather than re-running the work.\n\n\
             For long jobs: blocking: false parks it, then call again with a follow-up to steer or to collect. If you would relay the result as soon as it is ready rather than at your wrap-up, say so in the brief."
                .to_string(),
            schema.as_object().unwrap().clone(),
        )
    }
    ...
    /// Dispatch a brief to the sidekick.
    ///
    /// Reuses the delegate machinery rather than duplicating it: the sidekick is
    /// a delegate with its own model, a smaller parameter surface, and a brief
    /// framed for an independent peer.
    async fn handle_sidekick(
        &self,
        session_id: &str,
        arguments: Option<JsonObject>,
        cancellation_token: CancellationToken,
        ...
        let params: SidekickParams = arguments
            .map(|args| serde_json::from_value(serde_json::Value::Object(args)))
            .transpose()
            .map_err(|e| format!("Invalid parameters: {}", e))?
            .unwrap_or_default();
        ...
        let message = params
            ...
            .map(str::trim)
            .filter(|m| !m.is_empty())
            .ok_or_else(|| "sidekick requires a non-empty `message`".to_string())?;

        // The sidekick can run on its own provider and model when configured,
        // which is the point of fusion: a cheaper model for mechanical work.
        let model = params
            .model
            .filter(|m| !m.trim().is_empty())
            .or_else(crate::agents::local_fusion::sidekick_model_uid);
        let provider = crate::agents::local_fusion::sidekick_provider();

        let blocking = params.blocking.unwrap_or(true);
        let delegate_args = sidekick_delegate_args(message, blocking, provider, model);
    ...
    fn resolve_model_config(
        ...
        let env_model = crate::config::env_var("GOOSE_SUBAGENT_MODEL").ok();
    ...
    async fn resolve_provider(
        ...
        anyhow::Error,
    > {
        let env_provider = crate::config::env_var("GOOSE_SUBAGENT_PROVIDER").ok();
    ...
    fn resolve_max_turns(&self, session: &crate::session::Session) -> usize {
        session
            ...
            .or_else(|| {
                crate::config::env_var("GOOSE_SUBAGENT_MAX_TURNS")
                    .ok()
...
/// Build the delegate args for a sidekick dispatch.
///
/// The sidekick is a delegate with a brief framed for an independent peer;
/// `provider` and `model` are inserted only when set so the delegate's own
/// resolution (env, recipe, session) applies otherwise.
fn sidekick_delegate_args(
    message: &str,
    blocking: bool,
    ...
    args.insert(
        ...
        serde_json::Value::String(crate::agents::local_fusion::sidekick_brief(message)),
    );
    if let Some(provider) = provider {
        args.insert("provider".to_string(), serde_json::Value::String(provider));
    }
    if let Some(model) = model {
        args.insert("model".to_string(), serde_json::Value::String(model));
    }
    // blocking == sync delegate; non-blocking parks it as a background task
    args.insert("async".to_string(), serde_json::Value::Bool(!blocking));
    args
}
...
impl McpClientTrait for SummonClient {
    async fn list_tools(
        ...
        if !is_subagent {
            ...
            tools.push(self.create_sidekick_tool());
        }
...
You review code."#;
    ...
    #[test]
    fn sidekick_args_carry_provider_and_model() {
        let args = sidekick_delegate_args(
            "fix the parser",
            true,
            Some("karmx_custom".to_string()),
            Some("your-fast-model".to_string()),
        );
        assert_eq!(args["provider"], "karmx_custom");
        assert_eq!(args["model"], "your-fast-model");
        assert_eq!(args["async"], false);
        let instructions = args["instructions"].as_str().unwrap();
        assert!(instructions.contains("fix the parser"));
        assert!(instructions.contains("Report back"));
    }

    #[test]
    fn sidekick_args_omit_unset_provider_and_model() {
        let args = sidekick_delegate_args("do it", true, None, None);
        assert!(!args.contains_key("provider"));
        assert!(!args.contains_key("model"));
    }

    #[test]
    fn sidekick_args_nonblocking_is_async() {
        let args = sidekick_delegate_args("do it", false, None, None);
