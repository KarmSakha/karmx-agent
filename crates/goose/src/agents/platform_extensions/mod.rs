...
pub static PLATFORM_EXTENSIONS: Lazy<HashMap<&'static str, PlatformExtensionDef>> = Lazy::new(
    || {
        ...
        map.insert(
            browser_preview::EXTENSION_NAME,
            PlatformExtensionDef {
                name: browser_preview::EXTENSION_NAME,
                display_name: "Browser Preview",
                description:
                    "Open a live preview of a web app and read back the rendered DOM and console output",
                default_enabled: true,
                unprefixed_tools: true,
                hidden: false,
                client_factory: |ctx| {
                    Some(Box::new(
                        browser_preview::BrowserPreviewClient::new(ctx).unwrap(),
                    ))
                },
            },
        );
        ...
        map.insert(
            computer_control::EXTENSION_NAME,
            PlatformExtensionDef {
                name: computer_control::EXTENSION_NAME,
                display_name: "Computer Control",
                description: "Drive a real browser over CDP: click, type, scroll, drag, screenshot",
                default_enabled: true,
                unprefixed_tools: true,
                hidden: false,
                client_factory: |ctx| {
                    Some(Box::new(
                        computer_control::ComputerControlClient::new(ctx).unwrap(),
                    ))
                },
            },
        );
        ...
        map.insert(
            context_engine::EXTENSION_NAME,
            PlatformExtensionDef {
                name: context_engine::EXTENSION_NAME,
                display_name: "Context Engine",
                description:
                    "Search the codebase within a budget, enhance prompts, and rank context by relevance",
                default_enabled: true,
                unprefixed_tools: true,
                hidden: false,
                client_factory: |ctx| {
                    Some(Box::new(
                        context_engine::ContextEngineClient::new(ctx).unwrap(),
                    ))
                },
            },
        );
