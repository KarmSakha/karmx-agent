...
impl fmt::Display for GoosePlatform {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            GoosePlatform::GooseCli => write!(f, "karmx-cli"),
            GoosePlatform::GooseDesktop => write!(f, "karmx-desktop"),
        }
    }
}
...
pub struct Agent {
    ...
    /// A background compaction summary, carried across turns.
    ///
    /// `Arc` so a spawned summariser can write into the same slot the turn
    /// reads from.
    pub(super) pending_compaction: Arc<Mutex<Option<PendingCompaction>>>,
}

/// A summary produced in the background, waiting for a turn that can use it.
///
/// `started_at_tokens` is what makes this safe to apply later: a summary is
/// only valid for the history it was built from, so one that has been overtaken
/// by new messages is discarded rather than applied, which would silently drop
/// everything in between.
/// Not `Clone`: a prepared summary is consumed, never copied.
pub struct PendingCompaction {
    pub started_at_tokens: usize,
    /// The full result, not just the conversation: applying a compaction also
    /// has to record usage and emit `HistoryReplaced`, so those side effects
    /// are deferred to apply time rather than done in the background task.
    pub compaction: crate::context_mgmt::CompactionResult,
}

/// A background summary is discarded if the history grows past its start by
/// more than this many tokens. Derived from the headroom rather than fixed: the
/// headroom is the window in which we would have compacted anyway, so growth
/// beyond it means the summary no longer describes the conversation.
fn compaction_stale_after(t: &goose_context_management::CompactionThresholds) -> usize {
    t.hard.saturating_sub(t.apply)
}
...
impl Agent {
            ...
            async_stream::try_stream! {
                ...
                let result = {
                    let run = crate::session_context::with_session_id(
                        ...
                        run_goose(&machine, session_manager.as_ref(), &session_id, &emit, &self.hook_manager),
                    );
                    tokio::pin!(run);
                    loop {
                        tokio::select! {
    ...
    async fn reply_impl(
        ...
        let compaction_state = crate::context_mgmt::evaluate_compaction(
        ...
        Ok(Box::pin(async_stream::try_stream! {
            ...
            // Prefer a summary produced in the background on an earlier turn.
            //
            // Taking it out of the slot is what makes this non-blocking: the
            // summariser already ran, so the cost was paid against headroom
            // instead of against this turn.
            let ready = {
                let mut slot = self.pending_compaction.lock().await;
                match slot.as_ref() {
                    Some(pending)
                        if goose_context_management::summary_is_fresh(
                            pending.started_at_tokens,
                            compaction_state.current_tokens,
                            compaction_stale_after(&compaction_state.thresholds),
                        ) =>
                    {
                        slot.take().map(|pending| pending.compaction)
                    }
                    Some(_) => {
                        // Overtaken by newer history. Applying it would silently
                        // drop everything added since, so discard it.
                        *slot = None;
                        None
                    }
                    None => None,
            ...
            let final_conversation = if let Some(compaction) = ready {
                ...
                yield AgentEvent::Message(
                    Message::assistant().with_system_notification(
                        SystemNotificationType::InlineMessage,
                        "Applied the context summary prepared in the background",
                    )
                );

                compacted_conversation
            } else if !compaction_state.needs_compaction() {
                // Below the apply threshold. Start a summary now so it is ready
                // before it is needed, and carry on uncompacted.
                if compaction_state.should_start_summary() {
                    let provider = self.provider().await?;
                    let model_config = self.model_config_for_session(&session_config.id).await?;
                    let slot = self.pending_compaction.clone();
                    let session_id = session_config.id.clone();
                    let conversation_for_task = conversation.clone();
                    let started_at_tokens = compaction_state.current_tokens;
                    ...
                    tokio::spawn(async move {
                        ...
                        {
                            Ok(compaction) => {
                                *slot.lock().await = Some(PendingCompaction {
                                    started_at_tokens,
                                    compaction,
                                });
                            }
                            Err(e) => {
                                // A failed background summary is not fatal: the
                                // blocking path still runs at apply, so the only
                                // loss is the head start.
                    ...
                    yield AgentEvent::Message(
                        Message::assistant().with_system_notification(
                            SystemNotificationType::InlineMessage,
                            "Preparing a context summary in the background...",
                        )
                    );
                }
