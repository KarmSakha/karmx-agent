...
impl Paths {
    fn get_dir(dir_type: DirType) -> PathBuf {
        if let Some(base) = Self::path_root() {
            ...
            // NOTE: karmX uses its own config/data root so it does not read or
            // write goose's directories. The upstream "Block"/"goose" values were
            // kept only for backwards compatibility with existing goose installs.
            let strategy = choose_app_strategy(AppStrategyArgs {
                top_level_domain: "ai".to_string(),
                author: "karmX".to_string(),
                app_name: "karmx".to_string(),
            })
    ...
    pub(crate) fn path_root() -> Option<PathBuf> {
        // KARMX_PATH_ROOT takes precedence; GOOSE_PATH_ROOT is still honoured so
        // existing tooling keeps working.
        Self::validated_path_root(std::env::var_os("KARMX_PATH_ROOT"))
            .or_else(|| Self::validated_path_root(std::env::var_os("GOOSE_PATH_ROOT")))
    }
