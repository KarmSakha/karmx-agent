                ...
                match update {
                    ...
                    AcpUpdate::ToolCallStart { id, name, kind, raw_input } => {
                        ...
                        if reject_all_tools {
                            ...
                            // call. karmx.acp.kind preserves ACP's stable categorization for
...
mod tests {
    ...
    async fn messages_to_prompt_prepends_handoff_context_before_latest_user() {
        ...
        assert!(memo.starts_with(
            "Conversation context from karmx before this ACP provider session was created:"
        ));
    ...
    async fn a_budget_too_small_for_a_memo_keeps_the_claim() {
        ...
        assert!(
            ...
            "context that never left karmx must still be handed off later"
        );
    }
    ...
    fn tool_meta_pairs_external_dispatch_marker_with_acp_kind() {
        ...
        for (kind, expected) in cases {
            ...
            assert_eq!(
                tool_meta["karmx.acp.kind"],
                serde_json::Value::String(expected.to_string()),
                "karmx.acp.kind serialized wrong for kind={kind:?}"
            );
        }
    }
}
