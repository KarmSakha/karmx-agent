...
// WebSocket upgrades; karmx applies its richer loopback predicate before this.
const UPSTREAM_WS_ALLOWED_ORIGIN: &str = "http://karmx.local";
...
/// `karmx serve`. This allows the `x-secret-key` auth header the proxy routes
/// rely on.
...
/// or karmx-specific auxiliary routes.
...
/// The full standalone ACP server router used by `karmx serve`: ACP transport,
