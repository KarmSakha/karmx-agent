...
pub(super) const DEFAULT_PROVIDER_ID: &str = "karmx";
pub(super) const DEFAULT_PROVIDER_LABEL: &str = "karmX (Default)";
...
impl GooseAcpAgent {
    async fn on_initialize(
        ...
        Ok(InitializeResponse::new(args.protocol_version)
            .agent_info(Implementation::new("karmx", env!("CARGO_PKG_VERSION")))
            .agent_capabilities(capabilities)
            .auth_methods(vec![AuthMethod::Agent(
                AuthMethodAgent::new("karmx-provider", "Configure Provider")
                    .description("Run `karmx configure` to set up your AI provider and API key"),
            )]))
    }
