...
pub fn get_recipe_library_dir(is_global: bool) -> PathBuf {
    if is_global {
        ...
        env::current_dir().unwrap().join(".karmx/recipes")
    }
}
...
fn local_recipe_dirs() -> Vec<PathBuf> {
    ...
    if let Ok(recipe_path_env) = crate::config::env_var(GOOSE_RECIPE_PATH_ENV_VAR) {
    ...
    // Legacy goose recipe directory, kept for compatibility
    if let Ok(cwd) = env::current_dir() {
        local_dirs.push(cwd.join(".goose/recipes"));
    }
