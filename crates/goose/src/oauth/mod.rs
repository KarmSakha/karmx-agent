...
async fn complete_automatic_authorization(
    ...
    if crate::config::env_var_os("GOOSE_OAUTH_AUTOMATIC_CALLBACK").is_none() {
        return Ok(None);
    }
...
fn build_authorization_request(
    ...
    let mut request = AuthorizationRequest::new(redirect_uri).with_client_name("karmx");
    match static_client {
        Some(client) => {
...
pub async fn oauth_flow_with_challenge(
    ...
    let port = crate::config::env_var("GOOSE_OAUTH_CALLBACK_PORT")
        .ok()
