use super::undo::UndoActions;

/// Read a string-valued reserved meta key, ignoring a malformed one.
///
/// Never fails the tool call: losing a hint must not lose the result.
fn lift_string_meta(
...
pub struct ToolResponse {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub metadata: Option<ProviderMetadata>,
    /// Steps that reverse this call's effects.
    ///
    /// Empty means the call changed nothing. A list containing
    /// [`UndoAction::Irreversible`] means part of it cannot be taken back —
    /// which is not the same as having nothing to undo.
    #[serde(default, skip_serializing_if = "UndoActions::is_empty")]
    pub undo_actions: UndoActions,

    /// Content moved out of `content` because it was too large.
    ///
    /// The message in `content` is a pointer to a file; this holds the original
    /// text so it stays addressable rather than only reachable by parsing prose.
    /// `None` means nothing overflowed.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub overflow_content: Option<String>,

    /// Why the call failed, when the tool can say more than "it failed".
    ///
    /// Distinct from the `is_error` flag on the underlying result: a bool says
...
impl MessageContentBlock {
    ...
    pub fn tool_response_with_metadata<S: Into<String>>(
        ...
        // Lift any undo actions the tool declared in its result meta before the
        // result is consumed. Absent meta means the tool recorded no reverse
        // steps, which is not the same as declaring the call irreversible.
        let meta = tool_result
            .as_ref()
            .ok()
            .and_then(|r| r.meta.as_ref())
            .map(|m| &m.0);
        let undo_actions = UndoActions::from_tool_meta(meta);
        let overflow_content = lift_string_meta(meta, super::tool_meta::OVERFLOW_CONTENT);
        // A failure reason is only meaningful alongside a failure, so do not
        // record one on a successful call even if a tool set the key.
        let failure_reason = tool_result
            .as_ref()
            .ok()
            .filter(|r| r.is_error == Some(true))
            .and_then(|_| lift_string_meta(meta, super::tool_meta::FAILURE_REASON));
...
mod tests {
    ...
    use crate::conversation::undo::UndoActions;
    use crate::errors::ProviderError;
    use base64::Engine;
    use rmcp::model::{
