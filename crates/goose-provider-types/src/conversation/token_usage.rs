...
impl ProviderUsage {
    ...
    /// Why the turn ended, normalised across providers.
    ///
    /// `None` means the provider reported nothing, which is not the same as a
    /// clean stop — callers should not infer success from absence.
    ///
    /// Use [`crate::stop_reason::StopReason::is_resumable`] to decide whether a
    /// truncated turn can simply be continued.
    pub fn stop_reason(&self) -> Option<crate::stop_reason::StopReason> {
        self.finish_reasons
            .as_deref()
            .and_then(crate::stop_reason::StopReason::from_finish_reasons)
    }
