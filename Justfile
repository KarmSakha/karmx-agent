...
release-binary:
    ...
    cargo build --release -p goose-cli --bin karmx
    @just copy-binary
...
[windows]
release-windows:
    @powershell.exe -NoProfile -ExecutionPolicy Bypass -Command 'rustup target add x86_64-pc-windows-msvc; if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }; cargo build --release --target x86_64-pc-windows-msvc -p goose-cli --bin karmx; if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }; Write-Host "Windows executable created at ./target/x86_64-pc-windows-msvc/release/karmx.exe"'
...
copy-binary BUILD_MODE="release":
    ...
    @if [ -f ./target/{{BUILD_MODE}}/karmx ]; then \
        echo "Copying karmx CLI binary from target/{{BUILD_MODE}}..."; \
        rm -f ./ui/desktop/src/bin/goose; \
        cp -p ./target/{{BUILD_MODE}}/karmx ./ui/desktop/src/bin/goose; \
    else \
        echo "karmx CLI binary not found in target/{{BUILD_MODE}}"; \
        exit 1; \
    fi
...
copy-binary-intel:
    @rm -f ./ui/desktop/src/bin/goosed
    @if [ -f ./target/x86_64-apple-darwin/release/karmx ]; then \
        echo "Copying Intel karmx CLI binary to ui/desktop/src/bin..."; \
        rm -f ./ui/desktop/src/bin/goose; \
        cp -p ./target/x86_64-apple-darwin/release/karmx ./ui/desktop/src/bin/goose; \
    else \
        echo "Intel karmx CLI binary not found."; \
        exit 1; \
    fi
...
[windows]
copy-binary-windows:
    @powershell.exe -NoProfile -ExecutionPolicy Bypass -Command 'if (Test-Path ./target/x86_64-pc-windows-msvc/release/karmx.exe) { \
        ...
        Copy-Item -Path "./target/x86_64-pc-windows-msvc/release/karmx.exe" -Destination "./ui/desktop/src/bin/goose.exe" -Force; \
    } else { \
...
run-ui-playwright:
    ...
    RUN_DIR="$HOME/karmx-runs/$(date +%Y%m%d-%H%M%S)"
    mkdir -p "$RUN_DIR"
    echo "Using isolated directory: $RUN_DIR"
    cd ui/desktop && ENABLE_PLAYWRIGHT=true KARMX_PATH_ROOT="$RUN_DIR" GOOSE_PATH_ROOT="$RUN_DIR" pnpm run start-gui
...
run-server:
    ...
    KARMX_SERVER__SECRET_KEY="${KARMX_SERVER__SECRET_KEY:-${GOOSE_SERVER__SECRET_KEY:-test}}" cargo run -p goose-cli --bin karmx -- serve --platform desktop --enable-scheduler --host 127.0.0.1 --port 3000
...
mcp-conformance-build:
  cargo build -p goose-cli --bin karmx --bin mcp_conformance_driver
...
[doc("Run an MCP client conformance suite against karmx.")]
mcp-conformance version="2025-11-25" suite="all" conformance_version="0.2.0-alpha.10" build="true" baseline="":
  ...
  KARMX_DISABLE_KEYRING=1 npx -y @modelcontextprotocol/conformance@{{conformance_version}} client --command "target/debug/mcp_conformance_driver" --spec-version "{{version}}" --suite "{{suite}}" ${baseline_args[@]+"${baseline_args[@]}"}
